...
def main():
    ...
    for key in oss_keys:
        if key in in_progress:
            logging.warning(f"Skipping {key} - already in progress from previous run")
            continue
        try:
            ...
            if up_etag != encrypted_md5:
                ...
                if VALIDATE_UPLOADS:
                    ...
                    if validated_etag == oss_etag:
                        logging.info(
                            f"✓ {key} validated OK (ETag matches after upload)"
                        )
                    else:
                        logging.error(
                            f"✗ {key} validation failed (ETag mismatch after upload)"
                        )
