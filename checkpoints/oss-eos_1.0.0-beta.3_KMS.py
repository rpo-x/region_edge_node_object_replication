...
def envelope_encrypt(data: bytes, oss_etag: str) -> bytes:
    ...
    request = GenerateDataKeyRequest(
        key_id=KMS_KEY_ID, number_of_bytes=32
    )  # AES-256 DEK
    runtime = RuntimeOptions()
    response = kms_client.generate_data_key_with_options(request, runtime)
...
def main():
    ...
    for key in oss_keys:
        try:
            # HEAD OSS for ETag
            oss_head = oss_bucket.head_object(key)
            oss_etag = oss_head.etag.strip('"')

            need_upload = True
            if key in eos_keys:
                try:
                    # Range GET prefix from EOS
                    partial = eos_bucket.get_object(
            ...
            # Download full OSS, encrypt with prefix, upload
            ...
            # PUT to EOS (no metadata headers needed)
            up_result = eos_bucket.put_object(key, full_data)
